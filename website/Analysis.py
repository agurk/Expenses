...
class Analysis:
    ...
    def YearlySpend(self):
        ...
        for year in range (self.startYear, self.endYear):
            ...
            query = 'select amount, ccy, date from expenses e, classifications c, classificationdef cd where e.eid = c.eid and c.cid = cd.cid and c.cid = 27 and strftime(date) >= date(\'{0}\',\'start of year\') and strftime(date) <= date(\'{0}\',\'start of year\', \'+12 months\')'.format(date)
            cursor = conn.execute(query)
            for row in cursor:
                withholding[year] += self.fxValues.FXAmount(row[0],row[1],self.ccy,row[2])
        ...
        output['withholding'] = withholding
        return output 
